...
! MODULE DECLARATION
! ======================================================================================
MODULE MOD_VSCVSM_INTERNAL
	! Internal variables
	INTEGER :: I_STATE_DCGRID_G = -1
END MODULE MOD_VSCVSM_INTERNAL      

! ==========================================================
! SOLUTION OF NETWORK EQUATIONS
! ==========================================================   
SUBROUTINE TSCVSM(I_MACH,I_SLOT)
...
END SUBROUTINE TSCVSM

! ==========================================================
! SOLUTION OF DIFFERENTIAL EQUATIONS
! ==========================================================   
SUBROUTINE VSCVSM(I_MACH,I_SLOT)
	...
	USE MOD_MISC 											! use module of miscellaneous functions
	USE MOD_VSCVSM_INTERNAL									! use module for global VSCVSM-related variables
    ...
    INTEGER :: ierr, I_STATE_DCGRID, I_VAR_DCGRID, ILIMITPRIORITY
	...
	REAL :: TAU, H, D, KPFC, TPFC, KPOD, TPOD, KQ, TQ, RTVR, TTVR, KDCPSS, TUDCPSS, KD_P2, KD_I2, KPVI, SIGMAXR, PCMAX, PCMIN, QCMAX, QCMIN, ALOSS, BLOSS, CLOSSRECT, CLOSSINV, ICMAXSS, ICMAXIN, KD_P3, KD_I3, DELTAWMAX, KICPRIOR
	REAL :: hpu, dpu, kpfcpu, kpodpu, kqpu, rtvrpu, kdp2pu, kdi2pu, kpvipu, pcmaxpu, pcminpu, qcmaxpu, qcminpu, alosspu, blosspu, clossinvpu, clossrectpu, icmaxsspu, icmaxinpu, kdp3pu, kdi3pu
    ...
    IF (ierr .NE. 0) THEN
        WRITE (LPDEV,*) 'VSCVSM - ERROR: automatic DC-grid offset registration failed. IDGRID = ', IDGRID
        RETURN
    END IF
	...
	ALOSS = CON(I_CON+21) 		! constant converter loss coefficient (MW): ploss = alosspu + blosspu*ict + c*ict^2
	BLOSS = CON(I_CON+22) 		! linear converter loss coefficient (kV): ploss = alosspu + blosspu*ict + c*ict^2
	CLOSSRECT = CON(I_CON+23) 	! rectifier quadratic converter loss coefficient (ohm): ploss = alosspu + blosspu*ict + c*ict^2
	CLOSSINV = CON(I_CON+24) 	! constant converter loss coefficient (ohm): ploss = alosspu + blosspu*ict + c*ict^2
	...
	SELECT CASE (MODE)
		...
		CASE (1) 
			...
			IF ((I_VAR_DCGRID.LE.0).AND.(I_STATE_DCGRID.LE.0)) THEN
				WRITE (LPDEV,*) 'VSCVSM - CASE 1: No DCGRID VAR or STATE at bus ', NUMBUS(IB), ' with id ', MACHID(I_MACH), '.'
			...
			IF (ict.GT.icmaxsspu) THEN
				WRITE (LPDEV,*) 'VSCVSM - CASE 1: Current exceeds steady-state limit of ',icmaxsspu,' pu.'
			END IF		

			! DC-side voltage, current, power
			! Note that the VSC is behind a reactance not modelled in VSCVSM but in the power flow. blosspu should incude its resistance, rc
			...
			WRITE (LPDEV,*) 'VSCVSM - CASE 1: Converter ',IDXCONVERTER,' at bus ',NUMBUS(IB),' with id ',MACHID(I_MACH),' initialized. Initial conditions of states K+11/12 might be suspect.'  
			WRITE (LPDEV,*) 'VSCVSM - CASE 1: delta = ',xdelta,', deltamax = ',(asin(AIMAG(zc))+thetas),', thetas = ',thetas
		...
		CASE (5)
			! reporting mode
			WRITE (LPDEV,*) 'VSCVSM model at bus ', NUMBUS(IB)
